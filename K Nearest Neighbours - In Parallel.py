...
class CustomKNN:
    ...
    def predict(self, training_data, to_predict, k=3):
        if len(training_data) >= k:
            print(
                "K cannot be smaller than the total voting groups(ie. number of training data points)"
            )
            return
        ...
        for group in training_data:
            for features in training_data[group]:
                ...
                euclidean_distance = np.linalg.norm(
                    np.array(features) - np.array(to_predict)
                )
    ...
    def test(self, test_set, training_set):
        ...
        for group in test_set:
            arr[group] = pool.starmap(
                self.predict, zip(repeat(training_set), test_set[group], repeat(3))
            )
        e = time.clock()
